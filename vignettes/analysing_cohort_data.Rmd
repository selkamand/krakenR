...
remotes::install_github("selkamand/krakenR")
```
...
`krakenR` works by first reading a directory full of kraken reports into memory, running some basic computations, and writing the new data to an `SQLite ` database indexed to allow rapid analysis and visualization. 
...
```{r}
krakenreport_directory <- system.file(package="krakenR", "simulated_data/simulated_kraken_stdout_inc_zero_counts")
